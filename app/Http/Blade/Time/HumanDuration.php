...
class HumanDuration
{
    ...
    private $output;
    private string $daysRegex = '/([0-9\.]+)\s?(?:d|D)/';
    private string $hoursRegex = '/([0-9\.]+)\s?(?:h|H)/';
    private string $minutesRegex = '/(\d{1,2})\s?(?:m|M)/';
    private string $secondsRegex = '/(\d{1,2}(\.\d+)?)\s?(?:s|S)/';
    ...
    public function parse($duration)
    {
        ...
        if (is_numeric($duration)) {
            ...
            if ($this->hours >= $this->hoursPerDay) {
                ...
                $this->hours -= ($this->days * $this->hoursPerDay);
            }
     ...
     * @return string
     */
    public function formatted($duration = null, $zeroFill = false): string
    {
        if (null !== $duration) {
            $this->parse($duration);
        }
        ...
        } else if ($this->minutes > 0 || $hours > 0 || $zeroFill) {
            $this->output = '00';
        } else {
            $this->output = '0';
        }
     ...
     * @return string
     */
    public function humanize($duration = null): string
    {
        if (null !== $duration) {
            $this->parse($duration);
        }
    ...
    private function numberBreakdown($number, $returnUnsigned = false): array
    {
        $negative = 1;
